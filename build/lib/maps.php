...
function build_maps() {
  ...
  foreach (glob('maps/*.tmx') as $mapFile) {
    preg_match('#maps/(.*)\.\w+$#', $mapFile, $matches);
    $filelabel = $matches[1];
    ...
    foreach ($map->findMany('tileset') as $tileset) {
      ...
      if ($tilesetName === 'physicsTiles') {
        ...
        if (preg_match('#\.\./res/(.*)#', $image->attr['source'], $matches)) {
  ...
  file_put_contents('src/R.areas.js', "// DO NOT EDIT THIS FILE: it is automatically generated by build/lib/maps.php\n\n_.extend(R, \n" . json_encode($R) . "\n);\n");
