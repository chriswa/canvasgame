<?php
  ...
  $textureDirs = array_map('basename', array_filter(glob('textures/*'), 'is_dir'));
  ...
  foreach ($textureDirs as $textureName) {
    if (!file_exists("textures/$textureName.png"))  { $texturesNeedingPack[$textureName] = true; continue; }
    if (!file_exists("textures/$textureName.json")) { $texturesNeedingPack[$textureName] = true; continue; }
    $sliceFiles      = glob("textures/$textureName/*.png");
    $newestSliceTime = max(array_map('filemtime', $sliceFiles));
    $packTime        = @filemtime("textures/$textureName.png");
    if ($newestSliceTime > $packTime) {
      $texturesNeedingPack[$textureName] = true;
      continue;
    }
    $json = json_decode(file_get_contents("textures/$textureName.json"), true);
    ...
    <li>
      ...
      <?php else: ?>
        ...
        <?php if ($arbitrarySlice): ?>
          <span class="imgcontainer" style="display: inline-block; margin: 2px;"><img src="<?php echo "$BASEURL/textures/$textureName/$arbitrarySlice" ?>"></span>
        <?php else: ?>
          <span class="imgcontainer" style="display: inline-block; margin: 2px; color: white;">&nbsp;?&nbsp;</span>
        <?php endif ?>
